...
export default class AlbumScreen extends React.PureComponent<Props, State> {
    ...
    private async init(): Promise<void> {
        try {
            ...
            const response = await spotifyApi.getAlbumTracks(album.id, album.total_tracks);
            if (this._isMounted) {
    ...
    private openTrack(track: Album): void {
        const images = [
            {
                url: this.props.route.params.album.images[0]?.url as string
            }
        ];
                        ...
                        <View style={styles.overlay}>
                            ...
                            <View style={{ paddingHorizontal: 16 }}>
                                ...
                                <Text style={[themeStyles.fontColorMain, styles.lightLabel]}>
                                    <Text style={[themeStyles.fontColorMain, styles.strongLabel]}>{this.props.route.params.album?.total_tracks} </Text>
                                    Tracks
                                </Text>
                            </View>
                        </View>
                            ...
                            (item: Album, index: number) =>
                                ...
                                >
                                    <View style={styles.row}>
                                        <Image style={styles.cardCoverImage} source={{ uri: this.props.route.params.album.images[0]?.url as string }} />
                                        <View style={{ width: '80%' }}>
                                            <Text numberOfLines={1} style={[styles.title, themeStyles.fontColorMain]}>{item?.name}</Text>
                                            <View style={styles.row}>
                                                <Text style={[styles.name, themeStyles.fontColorSub]}>{item.artists[0]?.name} • </Text>
                                                <Text style={themeStyles.fontColorSub}>{formatTrackDuration((item as any)?.duration_ms)}</Text>
                                            </View>
                                        </View>
                                    </View>
